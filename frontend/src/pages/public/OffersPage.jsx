            ...
            {sortedVouchers.map((voucher) => {
              ...
              return (
                <div
                  ...
                  className={`group relative flex h-44 overflow-hidden rounded-[1.5rem] border transition-all duration-300 ${isExpired
                ...
                >
                  ...
                  <div className="relative flex flex-1 flex-col p-6">
                    <div className="mb-3">
                      <span className={`rounded-lg px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${isExpired ? 'bg-slate-200 text-slate-500' : themeColor === "orange" ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'
                        }`}>
                  ...
                  <div className="relative flex flex-col items-center justify-between py-1">
                    <div className={`absolute -top-3 left-1/2 -translate-x-1/2 size-5 rounded-full border shadow-inner z-10 ${isExpired ? 'border-slate-200 bg-slate-100' : 'border-orange-200 bg-[#f8fafc]'
                      }`} />
                    <div className="h-full w-px border-l border-dashed border-slate-300" />
                    <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 size-5 rounded-full border shadow-inner z-10 ${isExpired ? 'border-slate-200 bg-slate-100' : 'border-orange-200 bg-[#f8fafc]'
                      }`} />
                  </div>

                  {/* Right part: Action */}
                  <div className={`flex w-40 flex-col items-center justify-center p-6 text-center text-white ${isExpired ? 'bg-slate-400' : 'bg-orange-500'
                    }`}>
                    <div className="mb-4">
