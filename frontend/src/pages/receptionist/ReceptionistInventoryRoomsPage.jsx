...
export default function ReceptionistInventoryRoomsPage() {
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-1 text-[11px] font-black tracking-widest uppercase text-slate-400">
          Chọn phòng để thêm vật tư hoặc sao chép từ phòng khác
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {rooms.map((room) => (
            <RoomCard
              key={room.id}
              room={room}
              onClick={() => navigate(`/receptionist/inventory/${room.id}`)}
            />
          ))}
        </div>
      )}
