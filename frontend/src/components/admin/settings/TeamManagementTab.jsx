...
import { PencilLine, Plus, Search, ShieldCheck } from "lucide-react";
import { getRoles, shouldHideRoleFromAdminSettings } from "../../../api/admin/roleApi";
...
const TeamManagementTab = () => {
  ...
  const visibleRoles = useMemo(() => {
    ...
    if (!normalizedKeyword) return adminRoles;
  ...
  useEffect(() => {
    ...
    const loadRoles = async () => {
      ...
      try {
        ...
        if (isMounted) setMessage(readMessage(error, "Không thể tải danh sách vai trò."));
      } finally {
  ...
  const handleCreateRole = async () => {
    ...
    try {
      ...
      setMessage(readMessage(error, "Không thể tạo vai trò."));
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.25em] text-sky-500">Quản lý nhóm</p>
          <h2 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Phân quyền theo vai trò</h2>
          <p className="mt-2 max-w-2xl text-sm font-medium text-slate-500">
            Danh sách vai trò hiện có, số thành viên của từng vai trò và trang sửa riêng cho từng vai trò.
          </p>
        </div>
        ...
        <div className="flex w-full max-w-2xl flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          {canCreateRoles ? (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-sky-600 px-5 py-3 text-sm font-black text-white transition hover:bg-sky-700 disabled:opacity-60 disabled:cursor-not-allowed"
            >
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[760px] w-full">
            <thead className="border-b border-sky-100 bg-sky-50/80">
              <tr>
                <th className="px-8 py-5 text-left text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Vai trò</th>
                <th className="px-6 py-5 text-left text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Thành viên</th>
                <th className="px-6 py-5 text-center text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">Chỉnh sửa</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-sky-50">
              {isLoading ? (
                <tr>
                  <td colSpan={3} className="px-8 py-12 text-center text-sm font-semibold text-slate-400">Đang tải danh sách vai trò...</td>
                </tr>
              ) : visibleRoles.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-8 py-12 text-center text-sm font-semibold text-slate-400">Không tìm thấy vai trò phù hợp.</td>
                </tr>
              ) : (
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-xs font-medium text-slate-400">{role.description || "Chưa có mô tả vai trò."}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className="inline-flex rounded-full bg-sky-50 px-3 py-1.5 text-xs font-black text-sky-700">{role.userCount ?? 0} thành viên</span>
                    </td>
